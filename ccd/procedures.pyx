...
# (dates, observations, fitter_fn, quality, proc_params)
#ctypedef dict (*fit_type) (np.ndarray, np.ndarray, np.int, np.float, np.int)
#ctypedef list (*proc_type)(np.ndarray, np.ndarray, fit_type, slice, np.ndarray, np.ndarray, dict)
...
cdef tuple initialize(np.ndarray dates,
    ...
    while model_window.stop + meow_size < period.shape[0]:
        ...
        #models = [fitter_fn(period[model_window], spectrum, fit_max_iter, avg_days_yr, 4) for spectrum in spectral_obs[:, model_window]]
        models = []
        for spectrum in spectral_obs[:, model_window]:
            models.append(fitted_model(period[model_window], spectrum, fit_max_iter, avg_days_yr, 4))
...
cdef tuple lookforward(np.ndarray dates,
                       np.ndarray observations,
                       slice model_window,
                       np.ndarray processing_mask,
                       np.ndarray variogram,
    ...
    while model_window.stop + peek_size < period.shape[0] or models is None:
        ...
        if not models or model_window.stop - model_window.start < 24:
            ...
            models = [fitted_model(period[fit_window], spectrum,
...
def catch(dates, observations, processing_mask, model_window,
    ...
    models = [fitted_model(model_period, spectrum, fit_max_iter, avg_days_yr,
                        num_coef)
